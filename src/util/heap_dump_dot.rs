...
use crate::vm::{RootsWorkFactory, SlotVisitor, VMBinding};
use core::marker::Send;
use crossbeam::queue::SegQueue;
use std::sync::{Arc, Mutex};

#[derive(Clone)]
pub struct HeapValid;

#[derive(Clone, Debug)]
#[allow(dead_code)]
pub enum Error<VM: VMBinding> {
...
pub struct WorkFactory<Pred: ValidityPredicate<VM> + Clone, VM: VMBinding> {
    pub errors: Arc<Mutex<Vec<Error<VM>>>>,
    pub work_list: Arc<SegQueue<ObjectReference>>,
    pub pred: Pred,
}
...
where
    ...
    Pred: ValidityPredicate<VM> + Send + Clone,
{
    pub fn push_error(&mut self, error: Error<VM>) {
        self.errors.lock().expect("Failed to lock").push(error);
    }

    pub fn push_slot(&mut self, slot: <VM as VMBinding>::VMSlot) {
